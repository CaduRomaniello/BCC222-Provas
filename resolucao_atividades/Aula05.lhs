...
area (Circle _ x) = pi * (x^2)
area (Triangle (Point x1 y1) (Point x2 y2) (Point x3 y3)) = determinante (Triangle (Point x1 y1) (Point x2 y2) (Point x3 y3)) / 2
    where
        determinante (Triangle (Point x1 y1) (Point x2 y2) (Point x3 y3)) = 
            if (((x1 * y2) + (y1 * x3) + (x2 * y3)) - ((y2 * x3) + (y1 * x2) + (y3 * x1)) >= 0)
                then (((x1 * y2) + (y1 * x3) + (x2 * y3)) - ((y2 * x3) + (y1 * x2) + (y3 * x1)))
                else -(((x1 * y2) + (y1 * x3) + (x2 * y3)) - ((y2 * x3) + (y1 * x2) + (y3 * x1)))
\end{code}
...
{-listToTree :: [Int] -> IntTree
listToTree [] = ILeaf
listToTree [x] = INode x ILeaf ILeaf
listToTree list = INode raiz (listToTree menorQueRaiz) (listToTree maiorQueRaiz)
    where
        meio = length list `div` 2
        raiz = list !! meio
        menorQueRaiz = take meio list
        maiorQueRaiz = drop (meio + 1) list-}


listToTree :: IntList -> IntTree
listToTree INil = ILeaf
listToTree list = listToTree' list ILeaf
  where 
    listToTree' INil tree = tree
    listToTree' (ICons x xs) tree = listToTree' xs (insertIntTree x tree)
...
-- Nesse exercicio foi mais facil utilizar o tipo Client definido usando Reocrd Syntax
...
queremOfertas :: [Client] -> [Client]
queremOfertas lista = [a | a <- lista, (offers a) == True]
...
queremOfertas (x:xs) = 
    if (offers x)
        then (1 + (queremOfertas xs))
        else queremOfertas xs-}
\end{code}